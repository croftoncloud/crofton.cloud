...
def upload_static_site(s3_client, bucket_name, site_dir):
    '''
    Upload static site files to the specified S3 bucket.
    Args:
        s3_client: Boto3 S3 client.
        bucket_name: The S3 bucket name.
        site_dir: Path to the static site files.
    ...
    for root, _, files in os.walk(site_dir):
        for filename in files:
            ...
            relative_path = os.path.relpath(full_path, site_dir).replace('\\', '/')
            ...
            s3_client.upload_file(full_path, bucket_name, relative_path, ExtraArgs={'ContentType': content_type})
...
def main():
    ...
    parser.add_argument("--site-path", default="../website/out", help="Path to static site files to upload to S3.")
    ...
    with open("cloudformation/cfn-website-framework.yaml", "r", encoding="utf-8") as file:
    ...
    # Upload html to S3
    upload_static_site(s3_client, args.domain, args.site_path)
